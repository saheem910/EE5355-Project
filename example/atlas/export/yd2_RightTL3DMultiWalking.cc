...
static void output1(double *p_output1,const double *t,const double *a)
{
  ...
  p_output1[6]=t3*a[42] + 15.*t5*t6*a[44] + 20.*t7*t8*a[45] + 15.*t10*t9*a[46] + 6.*t11*t2*a[47] + t12*a[48] + 6.*t4*a[43]*t[0];
  p_output1[7]=t3*a[49] + 15.*t5*t6*a[51] + 20.*t7*t8*a[52] + 15.*t10*t9*a[53] + 6.*t11*t2*a[54] + t12*a[55] + 6.*t4*a[50]*t[0];
  p_output1[8]=t3*a[56] + 15.*t5*t6*a[58] + 20.*t7*t8*a[59] + 15.*t10*t9*a[60] + 6.*t11*t2*a[61] + t12*a[62] + 6.*t4*a[57]*t[0];
  p_output1[9]=t3*a[63] + 15.*t5*t6*a[65] + 20.*t7*t8*a[66] + 15.*t10*t9*a[67] + 6.*t11*t2*a[68] + t12*a[69] + 6.*t4*a[64]*t[0];
  p_output1[10]=t3*a[70] + 15.*t5*t6*a[72] + 20.*t7*t8*a[73] + 15.*t10*t9*a[74] + 6.*t11*t2*a[75] + t12*a[76] + 6.*t4*a[71]*t[0];
}
...
#include "yd2_RightTL3DMultiWalking.hh"
